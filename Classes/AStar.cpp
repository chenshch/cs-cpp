...
inline AStar::Node* AStar::IsExistInOpenList(const APoint &point)
{
...
inline bool AStar::IsExistInCloseList(const APoint &point)
{
	Node *node_ptr = maps_index_[point.row * num_row_ + point.col];
	return node_ptr ? node_ptr->state == IN_CLOSELIST : false;
}

bool AStar::IsCanReach(const APoint &target_point)
{
...
bool AStar::IsCanReached(const APoint &current_point, const APoint &target_point, bool allow_corner)
{
	...
	{
		return (IsCanReach(APoint(current_point.col + target_point.col - current_point.col, current_point.row))
				&& IsCanReach(APoint(current_point.col, current_point.row + target_point.row - current_point.row)));
	}

	return false;
}

void AStar::SearchCanReached(const APoint &current_point, bool allow_corner, std::vector<APoint> &surround_point)
{
	APoint target;
...
inline unsigned int AStar::CalculG(Node *parent, const APoint &current_point)
{
...
inline unsigned int AStar::CalculH(const APoint &current_point, const APoint &end_point)
{
...
void AStar::NotFoundNode(Node *current_point, Node *new_point, const APoint &end)
{
	new_point->parent = current_point;
...
std::deque<APoint> AStar::Search(const AStarDef &def)
{
	std::deque<APoint> search_path;
	if (ValidAStarDef(def))
	{
		Init(def);

		std::vector<APoint> around_point;
